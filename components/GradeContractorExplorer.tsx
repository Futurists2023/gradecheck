...
import type { CityGradeClassPath, ContractorWithGradings } from "@/types";

type GradeContractorExplorerProps = {
  city: string;
...
export function GradeContractorExplorer({
  city,
  classPaths,
  contractors,
  gradeLevel,
  province,
}: GradeContractorExplorerProps) {
  ...
  const classPathLookup = new Map(classPaths.map((path) => [path.class_code, path]));
  ...
  const visibleContractors = contractors
    .filter((contractor) => {
      ...
      return [contractor.contractor_name, contractor.trading_name ?? "", contractor.crs_number].some((value) =>
        value.toLowerCase().includes(deferredQuery),
      );
    })
    .sort((left, right) => {
      const statusComparison =
        sortStatuses(left.registration_status) - sortStatuses(right.registration_status);
      if (statusComparison !== 0) {
        return statusComparison;
      }

      const gradeClassCoverageComparison =
        right.gradings.filter((grading) => grading.grade_level === gradeLevel).length -
        left.gradings.filter((grading) => grading.grade_level === gradeLevel).length;
      if (gradeClassCoverageComparison !== 0) {
        return gradeClassCoverageComparison;
      }

      const gradingBreadthComparison = right.gradings.length - left.gradings.length;
      if (gradingBreadthComparison !== 0) {
        return gradingBreadthComparison;
      }

      return left.contractor_name.localeCompare(right.contractor_name);
    });

  const shortlistedContractors = contractors.filter((contractor) => shortlistIds.includes(contractor.crs_number));
  const comparedContractors = contractors.filter((contractor) => compareIds.includes(contractor.crs_number));
  ...
  const toggleShortlist = (crsNumber: string) => {
    void trackMonitoringEvent({
      ...
      metadata: { contractorCrs: crsNumber },
    });
    setShortlistIds((current) =>
      current.includes(crsNumber) ? current.filter((value) => value !== crsNumber) : [...current, crsNumber],
    );
  };

  const toggleCompare = (crsNumber: string) => {
    void trackMonitoringEvent({
      eventName: "compare_select",
      href: window.location.pathname,
      metadata: { contractorCrs: crsNumber },
    });
  ...
  const copyShortlistSummary = async () => {
    const summary = [
      `Shortlist for CIDB Grade ${gradeLevel} contractors in ${city}`,
      ...shortlistedContractors.map((contractor) => {
        const gradeClasses = contractor.gradings
          .filter((grading) => grading.grade_level === gradeLevel)
          .map((grading) => grading.class_code)
          .join(", ");
        ...
        return `- ${contractor.contractor_name} | CRS ${contractor.crs_number} | ${contractor.registration_status} | Grade ${gradeLevel} classes: ${gradeClasses || "None listed"} | Highest grade ${highestGrade}`;
      }),
    ].join("\n");
    ...
    <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {shortlistedContractors.length > 0 ? (
                      <button
                        ...
                        className={cn(buttonVariants({ variant: "default" }), "bg-white text-primary hover:bg-white/90")}
                      >
      ...
      {compareModalOpen ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="grade-compare-modal-title"
          ...
          >
            ...
            <div className="space-y-6">
              <div className="max-w-3xl space-y-2 pr-12">
                ...
                <h3 id="grade-compare-modal-title" className="font-serif text-3xl font-semibold">
                  Review up to three Grade {gradeLevel} candidates side-by-side
                </h3>
                <p className="text-muted-foreground">
                  Compare registration status, expiry, class coverage at this grade, overall grading breadth, and direct verification actions.
                </p>
              </div>
              ...
              <div className="grid gap-4 xl:grid-cols-3">
                {comparedContractors.map((contractor, index) => {
                  ...
                  const gradeClasses = contractor.gradings.filter((grading) => grading.grade_level === gradeLevel);
                  ...
                  return (
                    ...
                    >
                      <CardHeader>
                        ...
                        <CardTitle className="font-serif text-2xl">{contractor.contractor_name}</CardTitle>
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid gap-3 sm:grid-cols-2">
                          <div className="rounded-3xl border border-border bg-secondary/40 p-4">
                            ...
                            <p className="mt-2 text-lg font-semibold">{gradeClasses.length}</p>
                          </div>
                          <div className="rounded-3xl border border-border bg-secondary/40 p-4">
                            <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                              Highest grade held
                            </p>
                            <p className="mt-2 text-lg font-semibold">Grade {highestGrade}</p>
                          </div>
                        </div>
                        ...
                        <div className="rounded-3xl border border-border bg-white p-4">
                          ...
                          <p className="mt-2 text-sm font-medium">{formatDateTime(contractor.captured_at)}</p>
                        </div>

                        <div className="flex flex-wrap gap-2">
                          {gradeClasses.map((grading) => {
                            const path = classPathLookup.get(grading.class_code);
                            const label = `Grade ${grading.grade_level} ${grading.class_code}`;

                            return path?.has_public_leaf ? (
                              <Link
                                key={grading.id}
                                href={buildLeafHref(province, city, gradeLevel, grading.class_code)}
                                className="rounded-full border border-border bg-white px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-foreground hover:border-primary hover:text-primary"
                              >
                                {label}
                              </Link>
                            ) : (
                              <Badge key={grading.id} variant="outline" className="bg-white">
                                {label}
                              </Badge>
                            );
                          })}
      ...
      <div className="surface p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-muted-foreground">
              Screen every contractor holding Grade {gradeLevel}
            </p>
          </div>
          <div className="w-full max-w-md">
            <Input
              aria-label="Search contractors in this grade"
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          {(["all", "active", "pe", "expiring"] as ViewFilter[]).map((filterKey) => (
            <button
              key={filterKey}
              type="button"
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
          <span>{visibleContractors.length} contractors match the current view.</span>
          <span>
            Showing {(currentPage - 1) * RESULTS_PER_PAGE + 1}-
            {Math.min(currentPage * RESULTS_PER_PAGE, visibleContractors.length)}.
          </span>
          {view !== "all" ? <Badge variant="secondary">{FILTER_LABELS[view]}</Badge> : null}
          {deferredQuery ? <Badge variant="outline">Search: {query}</Badge> : null}
      ...
      {visibleContractors.length > 0 ? (
        <div className="space-y-5">
          <div className="grid gap-4">
            {paginatedContractors.map((contractor) => {
              const currentGradeClasses = contractor.gradings.filter((grading) => grading.grade_level === gradeLevel);
              const highestGrade = contractor.gradings.reduce(
                (highest, grading) => Math.max(highest, grading.grade_level),
                gradeLevel,
              );
              const broaderCoverageCount = contractor.gradings.length - currentGradeClasses.length;

              return (
                <article key={contractor.crs_number} className="surface overflow-hidden p-6">
                  <div className="space-y-5">
                    <div className="flex flex-wrap items-center gap-3">
                      <StatusBadge status={contractor.registration_status} />
                      <PEBadge pe_flag={contractor.pe_flag} />
                    ...
                    <div className="space-y-2">
                      ...
                      {contractor.trading_name ? (
                        <p className="text-sm text-muted-foreground">Trading name: {contractor.trading_name}</p>
                      ) : null}
                    </div>

                    <div className="flex flex-wrap gap-2 text-xs">
                      <span className="rounded-full bg-secondary/70 px-3 py-1.5 font-medium text-secondary-foreground">
                        Current-grade classes: {currentGradeClasses.length}
                      </span>
                      <span className="rounded-full bg-secondary/70 px-3 py-1.5 font-medium text-secondary-foreground">
                        Highest grade held: {highestGrade}
                      </span>
                      <span className="rounded-full bg-secondary/70 px-3 py-1.5 font-medium text-secondary-foreground">
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-bold uppercase tracking-[0.25em] text-muted-foreground">
                        Grade {gradeLevel} class coverage
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {currentGradeClasses.map((grading) => {
                          const path = classPathLookup.get(grading.class_code);
                          const label = `${grading.class_code} at Grade ${gradeLevel}`;
                          ...
                          return path?.has_public_leaf ? (
                            ...
                            <span
                              key={grading.id}
                              className="rounded-full border border-border bg-muted/55 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground"
                            >
                              {label}
                            </span>
                          );
                    ...
                    <div className="rounded-[1.35rem] border border-border/80 bg-muted/35 p-4">
                      <div className="grid gap-2 border-b border-border/70 pb-3 sm:grid-cols-[1.2fr_1fr] lg:grid-cols-[1.2fr_1fr_1fr]">
                      ...
                      <div className="pt-3">
                        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                          ...
                          <button
                            ...
                            disabled={
                              !compareIds.includes(contractor.crs_number) && compareIds.length >= 3
                            }
                            className={cn(
                              buttonVariants({
                          ...
                          <TrackingLink
                            href={buildContractorHref(contractor.crs_number, contractor.contractor_name)}
                            eventName="profile_open"
                            className={cn(buttonVariants({ variant: "default" }), "h-10 w-full text-xs")}
                          >
                          ...
                          <TrackingLink
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className={cn(buttonVariants({ variant: "outline" }), "h-10 w-full text-xs")}
                          >
                            Verify on CIDB
                          </TrackingLink>
                        </div>
        ...
        <div className="rounded-3xl border border-dashed border-border bg-white/80 p-10 text-center">
          <h3 className="font-serif text-2xl font-semibold">No contractors match this filter</h3>
          <p className="mt-3 text-muted-foreground">
            Try switching back to All matches or removing the search term to reopen the full screening set.
          </p>
        </div>
      )}
      ...
      <div className="surface p-6">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {classPaths.map((path) => (
            <Link
              key={path.class_code}
              href={buildClassCodeHref(path.class_code)}
              className="rounded-full border border-border bg-white px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-foreground hover:border-primary hover:text-primary"
            >
              {path.class_code} class guide
            </Link>
          ))}
        </div>
      </div>
